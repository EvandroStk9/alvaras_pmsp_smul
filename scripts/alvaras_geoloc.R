...
# 3 pares de sql_ajust aparecem duplicados
# todos contendo uma informaçãod e 2004 que foi setada com "-x" no dígito verificador
alvaras_por_lote %>%
  get_dupes(sql_ajust)

#
java_alvaras_por_endereco <- st_read(here("inputs", "0_transient", "Abrainc", "Alvaras",
                                          "ArquivosOriginais", "Agrupados",
                                          "20220530_pmsp_smul_licenciamento_alvaras_agrupamento.shp")) %>%
  mutate(cod_sql = case_when(str_detect(cod_sql, pattern = "\\+1") ~ NA_character_, # Mesmo tratamento feito em R
                             ...
                             TRUE ~ cod_sql),
         ind_sql_incra_null = if_else(str_detect(cod_sql, "(\\d)\\1{6,}"), TRUE, FALSE)) # se repete 6 vezes ou mais

#
java_alvaras <- st_read(here("inputs", "0_transient", "Abrainc", "Alvaras",
                             "ArquivosOriginais", "Individualizados",
                             "20220530_pmsp_smul_licenciamento_alvaras.shp"))

#
dir_tree(here("inputs", "0_transient", "GeoSampa", "Lotes"))

#
CRS <- "+proj=utm +zone=23 +south +ellps=WGS84 +datum=WGS84 +units=m +no_defs"

#
list_lotes <- list.dirs(here("inputs", "0_transient", "GeoSampa", "Lotes")) %>%
...
sf_geo_sp <- st_read(here("inputs", "0_transient", 
...
# 3453 lotes encontrados na base de Lotes/SQL's oficial de São Paulo
join_lotes <- lotes %>%
  select(sql_ajust) %>%
...
# 59 alvaras com 2 ou 3 lat-long de lotes correspondentes
...
# 7306 lotes sem correspondência 
...
# 3393 lotes distintos (31.7%) e 3322 sem nenhuma duplicata (31%)
...
join_java_3 <- java_alvaras_por_endereco %>%
  ...
  inner_join(alvaras_por_lote %>% select(sql_incra, sql_ajust, endereco_ultimo, endereco_registro), 
...
#
# geocode_osm <- tidygeocoder::geocode(
#   alvaras_por_lote_sem_geo %>%
#     mutate(endereco_key = if_else(!is.na(bairro),
#                                   paste0(endereco_ultimo, " ", bairro, " SAO PAULO SP"),
#                                   paste0(endereco_ultimo, " SAO PAULO SP")) %>%
#              str_remove_all(";|_|/") %>%
...
# Retorno de 592 de 880 (67.2%)
# 573 de 592 no perímetro de São Paulo (96.8%)
# sf_geocode_osm <- geocode_osm %>%
#   filter(!is.na(lat) | !is.na(long)) %>%
#   st_as_sf(coords = c("long", "lat"),
...
#
plot_map_arcgis <- ggplot(data = sf_geocode_arcgis) +
...
#
plotly::ggplotly(plot_map_arcgis)

#
geo_alvaras_por_lote_3 <- sf_geocode_arcgis %>%
  select(sql_incra, sql_ajust, endereco_ultimo)
...
# API comunidade ArcGIS: https://developers.arcgis.com/rest/
# geocode_arcgis_full <- tidygeocoder::geocode(
#   alvaras_por_lote %>%
#     filter(!sql_incra %in% c(geo_alvaras_por_lote_1$sql_incra)) %>%
#     mutate(endereco_key = if_else(!is.na(bairro),
#                                   paste0(endereco_ultimo, " ", bairro, " SAO PAULO SP"),
...
# Retorno de 890 de 890 (100%)
# Todos dentro do perímetro de São Paulo 
# sf_geocode_arcgis_full <- geocode_arcgis_full %>%
#   filter(!is.na(lat) | !is.na(long)) %>%
#   st_as_sf(coords = c("long", "lat"),
#            crs = "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs") %>% # 4326 CRS
#   st_transform(crs = 4674) %>%
...
# geo_alvaras_por_lote_4 <- sf_geocode_arcgis_full %>%
#   select(sql_incra, sql_ajust, endereco_ultimo)

#
# geo_alvaras_por_lote_alternative <- bind_rows(
#   geo_alvaras_por_lote_1,
#   geo_alvaras_por_lote_4
# ) %>%
#   right_join(alvaras_por_lote, by = c("sql_incra", "sql_ajust", "endereco_ultimo")) %>%
#   select(names(alvaras_por_lote)) %>%
#   select(-sql_ajust)
...
#
# sfarrow::st_write_parquet(geo_alvaras_por_lote_alternative, 
#                           here("inputs", "4_refined", "Alvaras", 
#                                "geo_alvaras_por_lote_arcgis.parquet"))
